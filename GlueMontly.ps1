...
aws s3 cp "employees_daily.py" "s3://$BUCKET_NAME/scripts/employees_montly.py" --quiet
...
$ArgsData = @{
    ...
    "--database"                         = $DATABASE
    "--table_name"                       = $TABLE
    "--output_path"                      = "s3://$BUCKET_NAME/processed/montly_realtime_employees/"
...
$CommandData = @{
    Name           = "glueetl"
    ScriptLocation = "s3://$BUCKET_NAME/scripts/employees_montly.py"
    PythonVersion  = "3"
}
